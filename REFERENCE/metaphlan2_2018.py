...
class TaxClade:
    ...
    def compute_abundance( self ):
        ...
        for m,n in sorted(self.markers2nreads.items(),key=lambda pars:pars[0]):
            ...
            if not self.avoid_disqm:
                ...
                for e in self.markers2exts[m]:
                    ...
                    ### SHIXU: 注意这里sum里面的list的元素是bool值，会被转换成1/0
        ...
        ### SHIXU: 如果满足条件的marker数量太少（<n_ripr=10），则从removed数据中恢复部分marker数据。
...
class TaxTree:
    ...
    key("taxonomy", "markers")
    {
        "taxonomy": 
        {
            "k__Kingdom|p__Pylum|...|s__Species_name|t__PRJNAxxxxxx": <int length>, 
            "xxx|xxx|t__GCF_0009xxxx": <int>, 
            ...
        },
