...
# SHIXU:
# All variables generated from expressions above:
# %hits = {
#     "genomeID": [
#         "Genus species", 
#         "GenomeName(may contains strain name)",
#         [sam_rec1_start_pos, sam_rec2_start_pos, ...]
#     ]
# }
# 
# %genomes = {
#     "Genus species" :[geno_len, geno_gc_percent]
# }
# 
# %str_genomes = {
#     "genomeID": genome_len
# } # Strain level genome length
# 
# (alignment ref) genomeID: gi|xxxxx|xxxxx
# 
# %reads = {
#     "readID": [
#         ["genomeID", "genomeID2", ...],
#         ["Genus species", "Genus species2", ...],
#         seq_gc
#     ]
# }
...
foreach $key (keys %hits)
{
    ...
    # SHIXU:
    # delete reference genomeID, which contains less than 5 SAM records, from alignment results .
...
# SHIXU:
# 以上操作把所有的不符合 islands 和 genome size 判断标准的 hits 记录全部删除. 接下来的一个 foreach 将各个 SAMRecord 的 read_seq_gc 值存入 Species 字典，其中做了一些细节处理，标记 “有问题” 的记录
...
foreach $key (keys %Species)
{
    ...
    # SHIXU: 
    # coverage ~ para1*exp(-0.5*((readGC-para2)/para3)^2)+para4+para5*readGC+para6*readGC^2+para7*readGC^3+para8*log(genomeGC)

    # SHIXU: 
    # para1=0.812093, para2=49.34331, para3=8.886807, para4=6.829778, para5=0.2642576, para6=-0.005291173, para7=0.00003188492, para8=-2.502158
...
# %Species = {
#     "Genus species": [
#         [read1_seq_gc, read2_seq_gc],
#         Q/NQ，
#         gc_norm_reads,
#         gc_norm_reads_per_MB_ref
#     ]
# }
# 输出所有 NQ 的结果
