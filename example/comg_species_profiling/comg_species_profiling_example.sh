...
${sparksubmit} ${sparkOptions} --class org.bgi.flexlab.metas.SOAPMetas ${soapmetas} --prof-pipe comg -i ${inputSampleList} -x ${referenceIndex} --large-index -o ${outputHDFSDir} -n ${partitionNumPerSample} -e "${bowtie2options}" --driver-tmp-dir ${driverTmp} --ana-lev species --ref-matrix ${refMatrix} --spe-gc ${speciesGenoGC} --align-executor-memory ${alnExeMem} --align-executor-number ${alnExeNum} --align-executor-cores ${alnExeCores} --prof-executor-memory ${profExeMem} --prof-executor-number ${profExeNum} --prof-executor-cores ${profExeCores} --prof-task-cpus ${profTaskCpus} 1>${WORKDIR}/running.o 2>${WORKDIR}/running.e
